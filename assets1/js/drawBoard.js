...
function drawGrid() {
  ...
  imageObj.src = "/squares/assets/images/sb.jpg";
...
function placeNumbers() {
  var arr1;
  var arr2;
  let horizontal = document.getElementById("horizontalNum");
  let vertical = document.getElementById("verticalNum");
  if (horizontal.value && vertical.value) {
    let horiz = horizontal.value.replace(/,/gi, "");
    let vert = vertical.value.replace(/,/gi, "");
    let horizInt = [...horiz].map((num) => parseInt(num));
    let vertInt = [...vert].map((num) => parseInt(num));
    arr1 = horizInt;
    arr2 = vertInt;
  } else {
    arr1 = ["X", "X", "X", "X", "X", "X", "X", "X", "X", "X"];
    arr2 = ["X", "X", "X", "X", "X", "X", "X", "X", "X", "X"];
  }
